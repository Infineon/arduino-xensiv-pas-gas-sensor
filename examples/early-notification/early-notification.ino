...
/**
 * In this example, the interrupt is used to control the  5V emitter 
 ...
 * 
 * 1. Direct hardware coupling of interrupt and 5V power supply.
 *    
 *    The interrupt signal is interfaced with a load control 
 *    circuitry which switches on/off the 5V signal. There is 
 ...
 * 
 * 2. MCU coupling of interrupt and 5V power supply.
 * 
 *    The interrupt signal is interfaced with the microcontroller,
 *    which uses another digital output to enable or disable the 
 *    5V emitter power supply. 
 * 
 * 3. Both direct 5V coupling and MCU interfacing of interrupt signal.
 * 
 *    Alternatively, both options can be combined: the interrupt is 
 *    directly interfaced to the 5V power supply enablement
